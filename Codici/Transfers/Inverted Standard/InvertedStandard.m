% This script generates the animation for the inverted standard transfer between
% initial and final orbit, with optimization for dv or dt.
% The maneuver starts by changing the orbit shape and then performs plane
% change and pericenter anomaly change.
...
maximize = 1;
export = 0;
filename = '.\Export\InvStandard';  % Filepath of saved frames (folder + frame name)
...
% First maneuver: First bitangent burn to change shape
[dv_1_1, dv_2_1, dt_2_1] = orbitShapeChange(a_i, e_i, a_f, e_f, om_i, om_i, mu, OM_i, i_i, 1);
[dv_1_2, dv_2_2, dt_2_2] = orbitShapeChange(a_i, e_i, a_f, e_f, om_i, om_i, mu, OM_i, i_i, 0);
dv_change_ae_1 = norm(dv_1_1)+norm(dv_2_1);
dv_change_ae_2 = norm(dv_1_2)+norm(dv_2_2);

% Find best shape change solution based on optimization
% Note: the choice is between apo-peri or peri-apo
...
[rr_semiVect] = semiOrb(rr_vect_complete,theta_i,angle2,a_i, e_i, i_i, OM_i, om_i, mu);
...
[dv_1, dv_2, dt_2] = orbitShapeChange(a_i, e_i, a_f, e_f, om_i, om_i, mu, OM_i, i_i, choice);

% Generate previous orbit data
if choice       % peri-apo
    [rr_1_i, vv_1_i] = parorb2rv(a_i, e_i, i_i, OM_i, om_i, 0, mu); 
else            % apo-peri
    [rr_1_i, vv_1_i] = parorb2rv(a_i, e_i, i_i, OM_i, om_i, pi, mu);
end
...
for k = 2:number_of_frames
    j = j+1;
    vv_1_i_post = vv_1_i+dv_1_vect(k, :);
...
% Update results
dt_1 = timeCalc(a_i, e_i, mu, theta_i, angle2);
dv_tot = dv_tot + norm(dv_1);
dt_tot = dt_tot + dt_1;


% Second maneuver: Second bitangent burn to change shape
angle1 = angle2;          % [rad], relative to the 1st orbit
angle2 = angle1+pi;       % [rad], relative to the 1st orbit


% Plot along orbit to reach second maneuver angle
[rr_vect_complete, ~] = parorb2rv(a_tras,e_tras,i_i,OM_i,om_i,theta_vect,mu); % complete orbit
...
dv_2_step = dv_2./number_of_frames;
dv_2_vect = (1:number_of_frames)'.*dv_2_step;

type = 1;
plot_temp = plot3(rr_semiVect(end,1),rr_semiVect(end,2),rr_semiVect(end,3),'or');
for k = 2:number_of_frames % without initial orbit
    j = j+1;  % global
    vv_2_tras_post = vv_2_tras+dv_2_vect(k,:);
...
% Third maneuver: Plane change
[dv_1_plane, dv_2_plane, theta_cambioPiano_1, theta_cambioPiano_2, om_1] = planeChange(a_f, e_f, i_i, OM_i, om_i, i_f, OM_f, mu);
...
angle1 = angle2;
angle2 = theta_cambioPiano;

[rr_vect_complete, ~] = parorb2rv(a_f,e_f,i_i,OM_i,om_i,theta_vect,mu); % complete orbit
[rr_semiVect] = semiOrb(rr_vect_complete,angle1,angle2,a_f, e_f, i_i, OM_i, om_i, mu);
...
for k = 2:number_of_frames % without initial orbit
    j = j+1;  % global
    [rr_orb, ~] = parorb2rv(a_f,e_f,i_vect.value(k),OM_vect.value(k),om_vect.value(k),theta_vect,mu);
...
% Update results
dt_3 = timeCalc(a_f, e_f, mu, angle1, angle2);
dv_tot = dv_tot+dv_3;
dt_tot = dt_tot+dt_3;
...
% Fourth maneuver: Pericenter anomaly change
[dv_4, theta_2_a, theta_2_b, om_2] = omChange_2(a_f, e_f, om_1, om_f, mu);
...
[rr_vect_complete, ~] = parorb2rv(a_f,e_f,i_f,OM_f,om_1,theta_vect,mu); % complete orbit
[rr_semiVect] = semiOrb(rr_vect_complete,angle1,angle2,a_f, e_f, i_f, OM_f, om_1, mu);
...
for k = 2:number_of_frames % without initial orbit
    j = j+1;  % global
    [rr_orb, ~] = parorb2rv(a_f,e_f,i_f,OM_f,om_vect.value(k),theta_vect,mu);
    exportPng(rr_orb,j,filename,export,type,s,frame_rotation_angle);
end
delete(plot_temp)
scatter3(rr_semiVect(end,1),rr_semiVect(end,2),rr_semiVect(end,3), 'blue', 'filled'); % Plot maneuver point
...
% Update results
dt_4 = timeCalc(a_f, e_f, mu, angle1, angle2);
dv_tot = dv_tot+dv_4;
dt_tot = dt_tot+dt_4;
